...
updated_taxa <- APCalign::create_taxonomic_update_lookup(taxa = input, identifier = "APCalign test", full = TRUE)
```

or, start with a csv file where there is a column of taxon names to align

``` {r, eval = FALSE, echo = TRUE}
taxon_list <- readr::read_csv("data/test_taxa.csv") #or load data through the R studio menu

updated_taxa <- APCalign::create_taxonomic_update_lookup(taxa = taxon_list$original_name, identifier = "APCalign test", full = TRUE)
```
