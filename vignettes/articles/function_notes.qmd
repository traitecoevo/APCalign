...
APCalign exports [9 functions](https://traitecoevo.github.io/APCalign/reference/index.html) to facilitate the alignment of submitted plant names to scientific names on the APC and APNI lists. They are listed in order of likelihood of use. 
...
APCalign::create_taxonomic_update_lookup(taxa = input, identifier = "APCalign test")
```
...
**example**:  
``` {r, eval = FALSE, echo = TRUE}
input <- c("Banksia serrata", "Banksia serrate", "Banksia cerrata", "Banksia serrrrata", "Dryandra")

APCalign::align_taxa(original_name = input, identifier = "APCalign test")
```
