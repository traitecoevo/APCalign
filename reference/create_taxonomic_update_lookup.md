...
Usage case: This is APCalign’s core function, merging together the
alignment and updating of taxonomy.
...
- taxa:
  ...
  A list of Australian plant species that needs to be reconciled with
  current taxonomy.

- stable_or_current_data:

  either "stable" for a consistent version, or "current" for the leading
  edge version.
...
- taxonomic_splits:
  ...
  How to handle one_to_many taxonomic matches. Default is "return_all".
  The other options are "collapse_to_higher_taxon" and
  "most_likely_species". most_likely_species defaults to the
  original_name if that name is accepted by the APC; this will be right
  for certain species subsets, but make errors in other cases, use with
  caution.

- full:

  logical for whether the full lookup table is returned or just key
  columns
...
- APNI_matches:
  ...
  Name matches to the APNI (Australian Plant Names Index) are turned off
  as a default.
...
- resources:
  ...
  These are the taxonomic resources used for cleaning, this will default
  to loading them from a local place on your computer. If this is to be
  called repeatedly, it's much faster to load the resources using
  [`load_taxonomic_resources`](https://traitecoevo.github.io/APCalign/reference/load_taxonomic_resources.md)
  separately and pass the data in.

- quiet:

  Logical to indicate whether to display messages while loading data and
  aligning taxa.

- output:

  file path to save the output. If this file already exists, this
  function will check if it's a subset of the species passed in and try
  to add to this file. This can be useful for large and growing
  projects.
...
- It uses first the function `align_taxa`, then the function
  `update_taxonomy` to achieve the output. The aligned name is plant
  name that has been aligned to a taxon name in the APC or APNI by the
  align_taxa function.
...
- If you will be running the function
  ...
  load the taxonomic resources separately using
...
- The argument taxonomic_splits allows you to choose the outcome for
  updating the names of taxa with ambiguous taxonomic histories; this
  applies to scientific names that were once attached to a more broadly
  circumscribed taxon concept, that was then split into several more
  narrowly circumscribed taxon concepts, one of which retains the
  original name. There are three options: most_likely_species returns
  the name that is retained, with alternative names documented in square
  brackets; return_all adds additional rows to the output, one for each
  possible taxon concept; collapse_to_higher_taxon returns the genus
  with possible names in square brackets.
